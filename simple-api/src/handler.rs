...
pub async fn health_checker_handler() -> WebResult<impl Reply> {
    Ok(with_status(json(&"PONG"), StatusCode::OK))
}
...
pub async fn create_todo_handler(mut body: Todo, db: DB) -> WebResult<impl Reply> {
    ...
    for todo in vec.iter() {
        if todo.title == body.title {
            ...
            return Ok(with_status(json(&err_response), StatusCode::CONFLICT));
        }
    }
...
pub async fn get_todo_handler(id: String, db: DB) -> WebResult<impl Reply> {
    ...
    return Ok(with_status(json(&err_response), StatusCode::NOT_FOUND));
}
...
pub async fn edit_todo_handler(
    ...
    for todo in vec.iter_mut() {
        if todo.id == Some(id.clone()) {
            ...
            let content = body.content.to_owned().unwrap_or(todo.title.to_owned());
            let payload = Todo {
                id: todo.id.to_owned(),
    ...
    Ok(with_status(json(&err_response), StatusCode::NOT_FOUND))
}
